                                                ...
                                                Targs&&... Fargs )
{
    ...
    */
    ...
    event_loop_.add_child_process( "packetshell", [&]() {
            ...
            interface_ioctl( SIOCSIFMTU, "ingress",
                             [] ( ifreq &ifr ) { ifr.ifr_mtu = PACKET_MTU; } );
...
template <class FerryQueueType>
template <typename... Targs>
void PacketShell<FerryQueueType>::start_uplink_without_nameserver( const string & shell_prefix,
                                                const vector< string > & command,
                                                Targs&&... Fargs )
{
    /* g++ bug 55914 makes this hard before version 4.9 */
    ...
    */
    ...
    event_loop_.add_child_process( "packetshell", [&]() {
            ...
            FerryQueueType uplink_queue { ferry_maker() };
            cout << "Enter the loop for without the nameserver" << endl;
            return inner_ferry.loop( uplink_queue, ingress_tun, egress_tun_ );
        }, false );  /* new network namespace */
}
...
template <class FerryQueueType>
template <typename... Targs>
void PacketShell<FerryQueueType>::start_downlink_without_nameserver( Targs&&... Fargs )
{
    /* g++ bug 55914 makes this hard before version 4.9 */
    BindWorkAround::bind<FerryQueueType, Targs&&...> ferry_maker( forward<Targs>( Fargs )... );
